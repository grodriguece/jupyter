...
def par_aud(ruta, datb, tablas, tipo):
    ...
    today = date.today()
    xls_file = tipo + today.strftime("get_ipython().run_line_magic("y%m%d")", " + \".xlsx\"")
    ...
    for index, row in df1.iterrows():  # table row iteration by audit2 column type
        ...
        if not pd.isna(row[tipo]):  # nan null values validation
            if line == 'LNCEL':
                pnglist = []
                tit = today.strftime("get_ipython().run_line_magic("y%m%d")", " + '_ParameterAudit'")
                xls_file = tit + ".xlsx"
                ...
                for i in range(0, n):  # loop for each carrier
                    ...
                    try:
                        ...
                        pyexcelerate_to_excel(wb, st, sheet_name='Carrier' + str(carr), index=True)
